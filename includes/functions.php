...
function getCurrentWeek() {
	...
	global $mysqli;
	$sql = "select distinct weekNum from " . DB_PREFIX . "schedule where year = " . $year . " and DATE_SUB(NOW(), INTERVAL 1 DAY) < gameTimeEastern order by weekNum limit 1";
	$query = $mysqli->query($sql);
	if ($query->num_rows > 0) {
		$row = $query->fetch_assoc();
		return $row['weekNum'];
	} else {
		$sql = "select max(weekNum) as weekNum from " . DB_PREFIX . "schedule where year = " . $year;
...
function getCutoffDateTime($week) {
	...
	global $mysqli;
	$sql = "select gameTimeEastern from " . DB_PREFIX . "schedule where weekNum = " . $week . " and year = " . $year . " and DATE_FORMAT(gameTimeEastern, '%W') = 'Sunday' order by gameTimeEastern limit 1;";
	$query = $mysqli->query($sql);
	if ($query->num_rows > 0) {
		$row = $query->fetch_assoc();
...
function getFirstGameTime($week) {
	...
	global $mysqli;
	$year = getSeasonYear();
	$sql = "select gameTimeEastern from " . DB_PREFIX . "schedule where weekNum = " . $week . " and year = " . $year ." order by gameTimeEastern limit 1";
	$query = $mysqli->query($sql);
	if ($query->num_rows > 0) {
		$row = $query->fetch_assoc();
		return $row['gameTimeEastern'];
...
function getGameIDByTeamName($week, $teamName) {
	...
	$sql .= "where weekNum = " . $week . " and s.year = " . $year;
...
function getGameIDByTeamID($week, $teamID) {
	...
	$sql .= "inner join " . DB_PREFIX . "teams t1 on s.homeID = t1.teamID ";
	$sql .= "inner join " . DB_PREFIX . "teams t2 on s.visitorID = t2.teamID ";
	$sql .= "where s.weekNum = " . $week . " and s.year = " . $year;
...
function getUserPicks($week, $userID) {
	...
	$sql .= "where s.weekNum = " . $week . " and s.year = " . $year . " and p.userID = " . $userID . ";";
	$query = $mysqli->query($sql);
...
function getUserScore($week, $userID) {
	...
	//get array of games
	$games = array();
	$sql = "select * from " . DB_PREFIX . "schedule where weekNum = " . $week . " and year = " . $year . " order by gameTimeEastern, gameID";
	$query = $mysqli->query($sql);
	while ($row = $query->fetch_assoc()) {
	...
	$sql .= "inner join " . DB_PREFIX . "schedule s on p.gameID = s.gameID ";
	$sql .= "where s.weekNum = " . $week . " and s.year = " . $year . " and u.userID = " . $user->userID . " ";
...
function getGameTotal($week) {
	...
	global $mysqli;
	$year = getSeasonYear();
	$sql = "select count(gameID) as gameTotal from " . DB_PREFIX . "schedule where weekNum = " . $week . " and year = " . $year;
	$query = $mysqli->query($sql);
	if ($query->num_rows > 0) {
		$row = $query->fetch_assoc();
...
function getLastCompletedWeek() {
	...
	$sql .= "from " . DB_PREFIX . "schedule s ";
	$sql .= "where s.gameTimeEastern < DATE_ADD(NOW(), INTERVAL " . SERVER_TIMEZONE_OFFSET . " HOUR) and s.year = " . $year . " ";
...
function calculateStats() {
	...
	for ($week = 1; $week <= $lastCompletedWeek; $week++) {
		...
		$sql .= "inner join " . DB_PREFIX . "users u on p.userID = u.userID ";
		$sql .= "inner join " . DB_PREFIX . "schedule s on p.gameID = s.gameID ";
		$sql .= "where s.weekNum = " . $week . " and s.year = " . $year . " and u.userName <> 'admin' ";
		$sql .= "order by u.lastname, u.firstname, s.gameTimeEastern";
		$query = $mysqli->query($sql);
		while ($row = $query->fetch_assoc()) {
...
function getTeamRecord($teamID,$week) {
	...
	$sql .= "where weekNum < " . $week . " and year = " . $year . " and final = 1";
...
function getSurvivorPrevPicks($userID) {
  	global $mysqli;
  	$year = getSeasonYear();
	$sql = "select survivor from " . DB_PREFIX . "picksummary where survivor is not NULL and userID = " . $userID . " and year = " . $year;
	$picks = array();
	$query = $mysqli->query($sql);
